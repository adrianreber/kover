...
/* $Id: PreferencesDialog.cc,v 1.13 2002/09/11 14:35:32 adrian Exp $ */
...
PreferencesDialog::PreferencesDialog (QWidget * parent,
    const QString & caption):KDialogBase (KJanusWidget::IconList, caption,
    Ok | Cancel, Ok, parent)
{
    this->parent = parent;
    setupCDDBPage ();
...
void PreferencesDialog::setupCDDBPage (void)
{
    ...
    cddb_widgets.cddb_server->setMinimumWidth (fontMetrics ().maxWidth () *
        10);
    ...
    cddb_widgets.proxy_server->setMinimumWidth (fontMetrics ().maxWidth () *
        10);
...
void PreferencesDialog::apply_settings ()
{
    ...
    if (!((cddb_widgets.cgi_path)->text ()).isEmpty ()) {
        ...
        globals.cgi_path =
            strdup (((cddb_widgets.cgi_path)->text ()).latin1 ());
    }
...
void PreferencesDialog::set_cddb ()
{
    ...
    cddb_widgets.proxy_port->setText (text.sprintf ("%d",
            globals.proxy_port));
...
void PreferencesDialog::setup_cddb_files_page (void)
{
    ...
    gbox->addMultiCellWidget (cddb_files_widgets.write_local_cddb, 1, 1, 0,
        5);
...
void PreferencesDialog::setup_font_page ()
{
    ...
    QLabel *label =
        new QLabel (i18n ("Content Font: "), group, "content_font");
    ...
    label =
        new QLabel (i18n ("Inlet Title Font: "), group, "inlet_title_font");
    gbox->addWidget (label, 2, 0);
...
{
    QFrame *page =
        addPage (i18n ("Miscellaneous"), i18n ("Various properties"),
