...
/* $Id: cdview.cc,v 1.7 2002/09/18 13:55:13 adrian Exp $ */
...
:QFrame (parent, name)
{
    ...
    connect (kover_file, SIGNAL (dataChanged (bool)),
        SLOT (dataChanged (bool)));
...
void CDView::drawInlet (QPainter * p, int X, int Y)
{
    ...
    p->fillRect (X, Y, BACK_HI + BACK_HS * 2, BACK_V,
        kover_file->backColor ());
    ...
    for (int i = 0; i < 3; i++) {
        if (!images[i].isNull ()) {
            ...
            case IMG_CENTER:
                ...
                case IMG_BACK_FULL:
                    if (previewMode)
                        p->setClipRect (X, Y, (BACK_HI + BACK_HS * 2),
                            BACK_V);
                    else
                        p->setClipRect ((int) (X * scale), (int) (Y * scale),
